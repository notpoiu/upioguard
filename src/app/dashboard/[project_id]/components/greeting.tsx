...
export default function GreetingDashText({name}: {name: string}) {
  ...
  return (
    <>
      ...
      <p>You are managing, <span className="font-bold">{data.name ?? "Unknown"}</span>. {data.project_type == "paid" ? "This script is paid." : "This script is freemium."} Created the <span className="font-bold">{format(data.creation_timestamp, "PPP")}</span>.</p>
    </>
  )
